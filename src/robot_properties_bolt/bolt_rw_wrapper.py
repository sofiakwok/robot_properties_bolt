...
class BoltRWRobot(PinBulletWrapper):
    def __init__(
        self,
        pos=None,
        orn=None,
        ...
        pybullet.setAdditionalSearchPath(BoltRWConfig.resources.package_path)
        self.urdf_path = BoltRWConfig.urdf_path
        self.simu_urdf_path = BoltRWConfig.simu_urdf_path
        if self.is_passive:
        ...
        self.pin_robot = BoltRWConfig.buildRobotWrapper()
        if self.is_passive:
            self.simu_pin_robot = BoltRWConfig.buildSimuRobotWrapper()
        ...
        super(BoltRWRobot, self).__init__(
            self.robotId,
            self.pin_robot,
            controlled_joints,
            self.end_effector_names,
    ...
    def get_state(self):
        ...
        if self.is_passive:
            q_simu, dq_simu = super(BoltRWRobot, self).get_state()
            q = np.concatenate([q_simu[0:10], q_simu[11:14]])
            dq = np.concatenate([dq_simu[0:9], dq_simu[10:13]])
            return q, dq
        else:
            q, dq = super(BoltRWRobot, self).get_state()
            return q, dq

    def _get_state_passive_ankle(self):
        # Returns a pinocchio like representation of the q, dq matrixes
        return super(BoltRWRobot, self).get_state()

    def reset_state(self, q, dq):
        if self.is_passive:
            q_simu = np.concatenate([q[0:10], [0.0], q[10:13], [0.0]])
            dq_simu = np.concatenate([dq[0:9], [0.0], dq[9:12], [0.0]])
            super(BoltRWRobot, self).reset_state(q_simu, dq_simu)
        else:
            super(BoltRWRobot, self).reset_state(q, dq)


    def send_joint_command(self, tau):
        if self.is_passive:
            tau_simu = np.concatenate([tau[0:3], [0.0], tau[3:6], [0.0]])
            super(BoltRWRobot, self).send_joint_command(tau_simu)
        else:
            super(BoltRWRobot, self).send_joint_command(tau)
